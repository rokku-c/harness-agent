...
test("gateway adapter emits only gateway endpoint and agent identity", () => {
  ...
  const plan = makeGatewayConfigAdapter("https://gateway.invalid/mcp-gateway").plan(agent, desired)
  expect(plan.revision).toBe(7)
  expect(JSON.stringify(plan.desired)).not.toContain("private-server")
  expect(plan.desired).toMatchObject({ mcpServers: { effectGateway: { url: "https://gateway.invalid/mcp-gateway", headers: { "x-agent-id": "agent-1" } } } })
})
