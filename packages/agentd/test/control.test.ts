...
test("agentd binds agents to named MCP sets and rejects stale revisions", () => {
  ...
  const binding = control.bindAgent("app:a1", ["coding"]), desired = control.desired("app:a1")
  expect(desired.sets[0].setId).toBe("coding"); expect(desired.servers[0].serverId).toBe("files")
  expect(() => control.reportApplied("app:a1", binding.revision - 1, {})).toThrow(AgentdError)
  expect(control.reportApplied("app:a1", binding.revision, { ok: true }).revision).toBe(binding.revision)
})
