...
const agent = { agentId: "app:a1", machineId: "m1", kind: "claude", version: "1", status: "online" as const }
...
const push = async (control: ReturnType<typeof makeAgentdControl>, machineId = "m1") => {
  ...
  const desired = control.desired("app:a1")
...
test("pushing a kernel and an app gives a receipt whose revision matches the push", async () => {
  ...
  const binding = control.bindBundles("app:a1", ["io.effect-agent.kernel@1.0.0", "board@1.0.0"])
  const { plan, applied } = await push(control)

  expect(plan.revision).toBe(binding.revision)
  expect(applied.metadata).toEqual({ agentId: "app:a1", revision: binding.revision })
  ...
  expect(control.reportApplied("app:a1", applied.metadata.revision, applied).revision).toBe(binding.revision)
})
...
test("a stale receipt is refused with 409, like every other agentd receipt", () => {
  ...
  control.publishBundle(app("1.0.0"))
  const binding = control.bindBundles("app:a1", ["board@1.0.0"])
  try {
    control.reportApplied("app:a1", binding.revision - 1, {})
...
test("a runtime the machine cannot provide is refused at plan time, naming the artifact", () => {
  ...
  control.bindBundles("app:a1", ["board@1.0.0"])

  const adapter = makeBundleArtifactAdapter()
  expect(() => adapter.plan(agent, control.desired("app:a1"))).toThrow(/cannot push board@1\.0\.0 to m1: .*runtime is "browser"/)
})
...
test("an abi the machine does not implement is refused, on the right line", () => {
  ...
  const adapter = makeBundleArtifactAdapter()
  try {
    adapter.plan(agent, control.desired("app:a1"))
...
test("a kernel needing another bootstrap line is refused on the bootstrap line, not the effect line", () => {
  ...
  control.bindBundles("app:a1", ["io.effect-agent.kernel@9.0.0"])
...
test("binding an older version is a plain binding change — rollback needs no new mechanism", async () => {
  ...
  control.bindBundles("app:a1", ["board@0.13.0"])
  const adapter = makeBundleArtifactAdapter()
  const plan = adapter.plan(agent, control.desired("app:a1"), forward)
...
test("the plan reports a diff a human can read, not just a new config", () => {
  ...
  const adapter = makeBundleArtifactAdapter()
  const first = adapter.plan(agent, control.desired("app:a1"))
  expect(first.changes).toEqual(["install board@0.13.0 (app)"])

  control.bindBundles("app:a1", ["board@0.13.0", "io.effect-agent.kernel@1.0.0"])
  const next = adapter.plan(agent, control.desired("app:a1"), first.desired)
...
test("the adapter validates its own contract and refuses a plan it did not produce", async () => {
  ...
  expect(() => adapter.validate({ artifacts: [{ bundleId: "k", version: "1", kind: "kernel", abi: KERNEL_ABI, runtimes: ["os"] }], metadata: { agentId: "app:a1", revision: 1 } })).toThrow(/bootstrapAbi/)
  expect(() => adapter.validate({ artifacts: [{ bundleId: "b", version: "1", kind: "app", abi: KERNEL_ABI, runtimes: ["os"], bootstrapAbi: BOOTSTRAP_ABI }], metadata: { agentId: "app:a1", revision: 1 } })).toThrow(/must not declare bootstrapAbi/)
  expect(() => adapter.validate({ artifacts: [{ bundleId: "b", version: "1", kind: "app", abi: KERNEL_ABI, runtimes: [] }], metadata: { agentId: "app:a1", revision: 1 } })).toThrow(/runtimes/)
  expect(() => adapter.validate({ artifacts: [{ bundleId: "b", version: "1", kind: "app", abi: KERNEL_ABI, runtimes: ["os"] }, { bundleId: "b", version: "1", kind: "app", abi: KERNEL_ABI, runtimes: ["os"] }], metadata: { agentId: "app:a1", revision: 1 } })).toThrow(/twice/)
  await expect(adapter.apply({ agentId: "app:a2", revision: 1, desired: { artifacts: [], metadata: { agentId: "app:a1", revision: 1 } }, changes: [] })).rejects.toThrow(/invalid adapter plan/)
})

test("a desired config naming no machine cannot be adjudicated, so it is refused", () => {
  const control = makeAgentdControl()
  control.registerMachine(machine()); control.registerAgent(agent)
  control.publishBundle(app("1.0.0")); control.bindBundles("app:a1", ["board@1.0.0"])
  const { machine: _machine, ...withoutMachine } = control.desired("app:a1")
  const adapter = makeBundleArtifactAdapter()
...
test("what agentd pushes is what the host can stage — no translation in between", async () => {
  ...
  control.bindBundles("app:a1", ["io.effect-agent.kernel@2.0.0"])

  const { plan } = await push(control)
  const pushed = control.desired("app:a1").bundles![0]
