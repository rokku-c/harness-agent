 ...
 *
 * The box is mutated in place (it is the session's state) and the turn is
 * resolved exactly once, whatever ends it: output, a spawn error, a timeout, or
 * the operator closing the session mid-turn.
 */
import { spawn, type ChildProcess } from "node:child_process"
import type { AgentKind } from "../kinds.ts"
import type { SendOutcome, SessionStatus } from "../flow.ts"
import type { UnifiedAgentConfig } from "../config-types.ts"

export interface CliBox {
