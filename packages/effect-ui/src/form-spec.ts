...
export const formToJsonSpec = (appId: string, schema?: JsonSchema, value?: Readonly<Record<string, unknown>>): Spec => {
  ...
  elements["cfg.title"] = { type: "Text", props: { value: `${appId} · config`, size: "4", weight: "bold" } }
  for (const strategy of ["apply", "restart"]) elements[`cfg.${strategy}`] = { type: "Button", props: { value: strategy === "apply" ? "Save and Apply" : "Save for Restart", strategy } }
  elements.root = { type: "Flex", props: { direction: "column", gap: "4" }, children: ["cfg.title", ...fields, "cfg.apply", "cfg.restart"] }
