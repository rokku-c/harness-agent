...
const surfaceWith = (authz = makeAuthz()): McpToolSurface => ({ gateway: make(undefined, undefined, { authz }).gateway, catalog: catalogWith() })
const asAlice = { authInfo: { clientId: "cli", extra: { "x-principal-kind": "user", "x-principal-id": "alice" } } }

test("a caller with no credentials is shown nothing", async () => {
  const surface = surfaceWith()
  expect(resolveSurfacePrincipal(surface, {}).principal).toBeUndefined()
  expect(await surfaceTools(surface, resolveSurfacePrincipal(surface, {}))).toEqual([])
  expect(await surfaceTools(surface, resolveSurfacePrincipal(surface, { headers: { "x-principal-kind": "system", "x-principal-id": "codex" } }))).toEqual([])
})

test("a caller sees exactly the tools its grants cover, in catalog order", async () => {
  const authz = makeAuthz()
  authz.grant({ subject: key, resource: "mcp://board/*", actions: ["call"] })
  const surface = surfaceWith(authz)
  const listed = await surfaceTools(surface, resolveSurfacePrincipal(surface, asAlice))
...
test("an advertised tool keeps its upstream schema, or gets a permissive one", async () => {
  const authz = makeAuthz()
...
test("what a caller is shown is exactly what it may call", async () => {
  ...
  const surface = surfaceWith(authz), gateway = surface.gateway
  const listed = (await surfaceTools(surface, resolveSurfacePrincipal(surface, asAlice))).map((entry) => entry.name)
...
test("the same request that lists a tool is refused when it calls a hidden one", async () => {
  ...
  authz.grant({ subject: key, resource: "mcp://board/*", actions: ["call"] })
  const surface = surfaceWith(authz)
  const hidden = surfaceTarget(surface, "files.read")!
  const result = await surface.gateway.handle({ callId: "s-hidden", principal: alice, serverId: hidden.serverId, tool: hidden.tool })
