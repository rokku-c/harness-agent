...
- F5: `whenRows` — the list and the press that empties it, drawn from the same
  ...
  fact. Then the app views and Settings, one slice per app; and a control whose
  gate path and send path are one name, so it is offered exactly when it can act
  (`Formal/Door.lean`).
