...
import { Context, Op, runDriver, Until, VercelAgent, type Binding } from "../src/index.js"
...
describe("Vercel agent settings", () => {
  test("allows maxOutputTokens override", async () => {
    ...
    await Effect.runPromise(runDriver(VercelAgent.make({ model, maxOutputTokens: { default: 16384, multiplier: 2, maxAttempts: 1, cap: 16384 } }), Context.with({ messages: [{ role: "user", content: "hello" }] }).withUntil(Until.stop)))
  ...
  test("injects system prompt into generateText when set", async () => {
    ...
    await Effect.runPromise(runDriver(VercelAgent.make({ model }),
      Context.with({ always: [{ _tag: "Always", text: "Be terse." }], messages: [{ role: "user", content: "hello" }] }).withUntil(Until.stop)))
    ...
    expect(JSON.stringify(call.prompt[1])).toContain("hello")
  })
  ...
  test("applies toolPrefix to injected op tool names", async () => {
    ...
    await Effect.runPromise(runDriver(VercelAgent.make({ model, toolPrefix: "ea_" }),
      Context.with({ messages: [{ role: "user", content: "hello" }] }).withUntil(Until.stop).withAccess([{ binding, write: false }])))
