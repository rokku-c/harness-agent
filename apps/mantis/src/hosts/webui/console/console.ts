 ...
 * while this shell wires the seams: console-as-operator approvals, A2UI
 * versioning, shared workspace, and the polled snapshot readers.
 */
...
import { acceptUi as acceptUiOp, restoreUi as restoreUiOp } from "./ui-ops.ts"
...
export class WebConsole {
  ...
  constructor(options: WebConsoleOptions) {
    ...
    this.ui = new UiStore(options.uiDir)
    ...
    this.host = buildConsoleHost({
      ...
      approval,
      acceptUi: (messages, author) => acceptUiOp(this.ui, this.bus, messages, author),
  ...
  readonly acceptUi = (messages: ReadonlyArray<A2uiMessage>, author: string): void => acceptUiOp(this.ui, this.bus, messages, author)
  ...
  readonly restoreUi = (version: number): { ok: boolean; detail?: string } => restoreUiOp(this.ui, version)
  ...
  readonly state = () => consoleState(this.host, this.#ledger, this.ui, this.gate, this.startedAt)
}
