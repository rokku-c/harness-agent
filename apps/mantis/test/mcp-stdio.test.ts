...
describe("mantis MCP stdio server (external-agent contract)", () => {
  test("stdout stays clean JSON-RPC and the full tool surface is advertised", async () => {
    ...
    try {
      ...
      expect(mantis.length).toBeGreaterThanOrEqual(11)
      for (const must of ["mantis_chat", "mantis_conversation", "mantis_pending", "mantis_approve", "mantis_workspace", "mantis_workspace_write", "mantis_workspace_update", "mantis_workspace_delete", "mantis_events", "mantis_state"])
