...
import { startAiGateway } from "../src/main.ts"
...
test("proxies OpenAI-compatible requests with controlled injection and audit", async () => {
  ...
  const auditFile = join(tmpdir(), `ai-gateway-${crypto.randomUUID()}.jsonl`)
  const gateway = startAiGateway({ port: 0, upstreamBase: upstream.url.toString(), auditFile, rules: [{ ruleId: "coder-control", match: { agent: "coder" }, inject: { content: "Stay in scope." } }] })
  try {
...
test("exposes health without contacting the upstream", async () => {
  const gateway = startAiGateway({ port: 0, upstreamBase: "http://127.0.0.1:1", auditFile: join(tmpdir(), `ai-health-${crypto.randomUUID()}.jsonl`) })
