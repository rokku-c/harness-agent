...
test("agentd exposes desired config, validates applied revisions, and reports drift", async () => {
  ...
  try {
    const desired = await (await host.handle(new Request("http://host/agentd/desired?agentId=app:a1"))).json() as { desired: { revision: number } }
    expect(desired.desired.revision).toBeGreaterThan(0)
    const stale = await host.handle(new Request("http://host/agentd/report", { method: "POST", body: JSON.stringify({ agentId: "app:a1", revision: 0 }) }))
    expect(stale.status).toBe(409)
    const accepted = await host.handle(new Request("http://host/agentd/report", { method: "POST", body: JSON.stringify({ agentId: "app:a1", revision: desired.desired.revision, state: { applied: true } }) }))
    ...
    expect(status.agents[0]!.applied).toMatchObject({ agentId: "app:a1", revision: desired.desired.revision })
    expect(status.agents[0]!.desired.agent.agentId).toBe("app:a1")
