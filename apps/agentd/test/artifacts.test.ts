...
test("agentd pushes code artifacts and refuses one the machine cannot run", async () => {
  ...
  const config = effectConfig.schema.parse({
    ...
    bundleBindings: [{ agentId: "app:a1", bundleIds: ["io.effect-agent.kernel@1.0.0", "board@1.0.0"] }],
  })
  await host.register({ ...createAgentdPlugin(() => config), routes: effectApp.routes })
  try {
    const plan = await (await host.handle(new Request("http://host/agentd/plan?agentId=app:a1"))).json() as {
    ...
    const accepted = await host.handle(new Request("http://host/agentd/report", { method: "POST", body: JSON.stringify({ agentId: "app:a1", revision: plan.revision }) }))
...
test("rebinding to another artifact version is a binding change, and the plan shows the diff", async () => {
  const plugin = createAgentdPlugin(() => effectConfig.schema.parse({
    ...
    bundleBindings: [{ agentId: "app:a1", bundleIds: ["board@1.0.0"] }],
  }))
  ...
  try {
    ...
    const forward = await (await host.handle(new Request("http://host/agentd/plan?agentId=app:a1"))).json() as { revision: number; desired: unknown }
    expect((forward.desired as { artifacts: ReadonlyArray<{ version: string }> }).artifacts[0].version).toBe("1.0.0")
    bind.handler({ agentId: "app:a1", bundleIds: ["board@0.13.0"] })
    const back = planTool.handler({ agentId: "app:a1", reported: forward.desired }) as { changes: readonly string[]; desired: { artifacts: ReadonlyArray<{ version: string }> } }
...
test("a push of an artifact the machine cannot run is refused with the reason, not a generic error", async () => {
  ...
  await host.register({ ...createAgentdPlugin(() => config), routes: effectApp.routes })
  try {
    const refused = await host.handle(new Request("http://host/agentd/plan?agentId=app:a1"))
...
test("a config that conflates the two ABI lines is rejected at parse time", () => {
  const base = { agents: [{ agentId: "app:a1", machineId: "m1", kind: "effect", version: "1" }] }
