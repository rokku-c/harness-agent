 ...
 * read back through `ui_get_runtime_state` - a page that named its own theme
 * could disagree with the one actually drawing it. The renderer was once the
 * second of those; it is not one any more, because the host has exactly one and
 * a setting with a single value is not a setting.
 */
