  ...
  const render = (item: WorkItem, depth: number): JSX.Element => (
    <Stack key={item.itemId} gap={4} ml={depth * 18}>
      <Paper withBorder p="xs" onClick={() => onOpen(item.itemId)} style={{ cursor: "pointer" }}>
        <Group justify="space-between"><Text size="sm" fw={item.kind !== "leaf" ? 600 : 400}>{item.title}</Text><Group gap={8}><Text size="xs" c="dimmed">{Math.round(progress(item) * 100)}%</Text><Text size="xs" c={stateColor[item.state] ?? "dimmed"}>{item.state}</Text></Group></Group>
      </Paper>
