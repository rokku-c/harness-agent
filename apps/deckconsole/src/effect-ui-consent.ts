/**
 * The consent policy a session opens under, and the four tool names it may run
 * without asking.
 *
 * The policy half is why the `new` screen is not three fields. `flows.md` §7.8
 * records that the deck's legacy page opened a session under a consent policy and
 * that the declarative `new` screen had only kind, id and prompt — so the console
