...
import { makeLiveSets, makeRegistrySetResolver, makeToolCatalog, type McpSetFacts, type McpSetSlot } from "@effect-agent/mcp-gateway"
...
import { previewAccess } from "../src/access-preview.ts"
...
/**
 * The center's own reading, as agentd hands it over: a revision over a value and
 * never a copy of it. Handing the preview a *config* would be the second store
 * this design removes, so the test hands over the same thing the center does.
 */
const centerOf = (facts: McpSetFacts): McpSetSlot => ({ provide: () => {}, source: () => ({ facts: () => facts }) })

/** The engine the plugin builds, built the same way — the preview asks this one, and asks it about the tools the door offers. */
const surfacesOf = (facts: McpSetFacts) => {
  ...
  return { registry, sets: makeLiveSets(centerOf(facts), makeRegistrySetResolver(registry)), offered }
}

const safe = { setId: "safe", name: "Safe", servers: ["files"], allowTools: ["read"] }
const danger = { setId: "danger", name: "Danger", servers: ["missing"], denyTools: ["write"] }
const bound = (sets: McpSetFacts["sets"], agentId = "app:agent-1"): McpSetFacts =>
  ({ revision: 1, sets, bindings: [{ agentId, setIds: sets.map((set) => set.setId) }] })
const both = bound([safe, danger])

test("previewAccess reads its verdict off the store the door decides with", () => {
  const allowed = previewAccess(surfacesOf(both), "app:agent-1", "files.read")
  ...
  const denied = previewAccess(surfacesOf(both), "app:agent-1", "files.write")
  expect(denied.allowed).toBe(false)
  expect(denied.reasons).toEqual(["safe allowlist does not include write"])

  // the denying set alone: it decides, and its own deny is the reason
  const refused = previewAccess(surfacesOf(bound([{ ...safe, allowTools: undefined, denyTools: ["write"] }])), "app:agent-1", "files.write")
  ...
  const unreachable = previewAccess(surfacesOf(bound([{ setId: "safe", name: "Safe", servers: ["missing"] }])), "app:agent-1", "files.read")
  ...
  const elsewhere = bound([{ setId: "other", name: "Other", servers: ["cache"] }])
  ...
  expect(previewAccess(surfacesOf(both), "app:agent-1", "files.remove").reasons).toEqual(["no server offers files.remove"])
  ...
  const unbound = previewAccess(surfacesOf(both), "nobody", "files.read")
  ...
  expect(previewAccess(surfacesOf(both), "nobody").reasons).toEqual(["no set bound to this agent"])
})
...
test("gateway serves access previews and exposes an audit surface", async () => {
  ...
  const plugin = createMcpGatewayPlugin(() => config, { mcpRegistry: registry, mcpSets: centerOf(both), fetch })
  await host.register({ ...plugin, routes: [{ path: "/mcp-gateway", match: "prefix" }] })
  try {
