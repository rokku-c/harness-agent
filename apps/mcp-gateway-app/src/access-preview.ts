...
import { refusalAnswer, type Fixing } from "./access-reasons.ts"
...
export const previewAccess = (surfaces: AccessSurfaces, agentId: string, tool?: string): AccessPreview => {
  ...
  const refusal = allowed ? undefined : refusalAnswer({ sets, agentId, bound, tool, entry, resolution })
  return {
    agentId, bound, allowed, sets: boundSets(surfaces, agentId),
    reasons: refusal?.reasons ?? [],
    ...(refusal?.fixing === undefined ? {} : { fixing: refusal.fixing }),
  }
}
